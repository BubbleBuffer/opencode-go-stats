...
(() => {
  ...
  function buildSection(title, desc, rows, cols) {
    ...
    for (const row of rows) {
      ...
      for (const cell of row) {
        ...
        else if (cell && typeof cell === "object" && cell.nodeType) td.appendChild(cell);
  ...
  // src/charts.ts
  var COLORS = ["#007aff", "#ff9f0a", "#30d158", "#ff375f", "#5e5ce6", "#64d2ff", "#ffd60a", "#bf5af2", "#ff6482", "#32d74b"];
  var dateRanges = [
    { label: "All", fn: (r) => !!r.timeCreated },
    { label: "Today", fn: (r) => new Date(r.timeCreated).toDateString() === (/* @__PURE__ */ new Date()).toDateString() },
  ...
  async function loadChartJS() {
    ...
    return new Promise((resolve) => {
      ...
      s.onerror = () => {
        console.warn("[oc-stats] failed to load Chart.js from CDN");
        resolve(null);
      };
      document.head.appendChild(s);
    });
  }
  function renderCharts(allRecords, getStats, applyFilter, target) {
  ...
  `;
    ...
    function renderCostByModel() {
      ...
      return new $Chart(canvas, {
        type: "bar",
        data: {
          labels: byModel.map(([m2]) => m2),
          datasets: [
            { label: "Input", data: byModel.map(([m2, sm]) => +(sm.inputTokens / 1e6 * (ep[m2]?.inputTokens || 0)).toFixed(6)), backgroundColor: COLORS[0] },
            { label: "Output", data: byModel.map(([m2, sm]) => +(sm.outputTokens / 1e6 * (ep[m2]?.outputTokens || 0)).toFixed(6)), backgroundColor: COLORS[1] },
            { label: "Cache Rd", data: byModel.map(([m2, sm]) => +(sm.cacheReadTokens / 1e6 * (ep[m2]?.cacheReadTokens || 0)).toFixed(6)), backgroundColor: COLORS[3] },
        ...
        options: {
          ...
          scales: {
            x: { stacked: true, ticks: { color: "#6e6e73", callback: (v) => "$" + v }, grid: { color: "#2c2c2e" } },
    ...
    function renderDailyCost() {
      ...
      let c = 0;
      for (const d of days) {
        for (const m2 of models) c += daily[d]?.[m2] || 0;
      ...
      return new $Chart(canvas, {
        ...
        data: {
          ...
          datasets: [
            ...models.map((m2, i) => ({
              label: m2,
              data: days.map((d) => +(daily[d]?.[m2] || 0).toFixed(6)),
            ...
            tooltip: {
              callbacks: {
                label: (ctx) => {
                  ...
                  const dayTotal = days.length ? models.reduce((sum, m2) => sum + (daily[days[ctx.dataIndex]]?.[m2] || 0), 0) : 0;
    ...
    prevBtn.addEventListener("click", () => {
      if (chartIdx > 0) showChart(chartIdx - 1);
    });
    nextBtn.addEventListener("click", () => {
      if (chartIdx < charts.length - 1) showChart(chartIdx + 1);
    });
    showChart(0);
  }
